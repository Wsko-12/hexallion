...
class FieldCeil {
  constructor(properties) {
    ...
    if (this.cityCeil) {
      ...
      this.city = city;
      MAIN.gameData.cities[this.type] = city;
    };
  };
  ...
  findNeighbours() {
    ...
    if (this.indexes.z < 5) {
      ...
      if (MAIN.gameData.map[this.indexes.z - 1]) {
        if (MAIN.gameData.map[this.indexes.z - 1][this.indexes.x]) {
          this.neighbours[0] = MAIN.gameData.map[this.indexes.z - 1][this.indexes.x];
        };
      };
      if (MAIN.gameData.map[this.indexes.z][this.indexes.x + 1]) {
        this.neighbours[1] = MAIN.gameData.map[this.indexes.z][this.indexes.x + 1];
      };
      if (MAIN.gameData.map[this.indexes.z + 1][this.indexes.x + 1]) {
        this.neighbours[2] = MAIN.gameData.map[this.indexes.z + 1][this.indexes.x + 1];
      };
      if (MAIN.gameData.map[this.indexes.z + 1][this.indexes.x]) {
        this.neighbours[3] = MAIN.gameData.map[this.indexes.z + 1][this.indexes.x];
      };
      if (MAIN.gameData.map[this.indexes.z][this.indexes.x - 1]) {
        this.neighbours[4] = MAIN.gameData.map[this.indexes.z][this.indexes.x - 1];
      };
      if (MAIN.gameData.map[this.indexes.z - 1]) {
        if (MAIN.gameData.map[this.indexes.z - 1][this.indexes.x - 1]) {
          this.neighbours[5] = MAIN.gameData.map[this.indexes.z - 1][this.indexes.x - 1];
        };
      };
    };
    if (this.indexes.z === 5) {
      ...
      if (MAIN.gameData.map[this.indexes.z + 1][this.indexes.x]) {
        this.neighbours[2] = MAIN.gameData.map[this.indexes.z + 1][this.indexes.x];
      };
      if (MAIN.gameData.map[this.indexes.z + 1][this.indexes.x - 1]) {
        this.neighbours[3] = MAIN.gameData.map[this.indexes.z + 1][this.indexes.x - 1];
      };
      if (MAIN.gameData.map[this.indexes.z][this.indexes.x - 1]) {
        this.neighbours[4] = MAIN.gameData.map[this.indexes.z][this.indexes.x - 1];
      };
    ...
    if (this.indexes.z > 5) {
      if (MAIN.gameData.map[this.indexes.z - 1][this.indexes.x + 1]) {
        this.neighbours[0] = MAIN.gameData.map[this.indexes.z - 1][this.indexes.x + 1];
      };
      ...
      if (MAIN.gameData.map[this.indexes.z]) {
        if (MAIN.gameData.map[this.indexes.z][this.indexes.x - 1]) {
          this.neighbours[4] = MAIN.gameData.map[this.indexes.z][this.indexes.x - 1];
        };
      };
      if (MAIN.gameData.map[this.indexes.z - 1][this.indexes.x]) {
        this.neighbours[5] = MAIN.gameData.map[this.indexes.z - 1][this.indexes.x];
      };
    };
  };
  ...
  onClick(intersectCoords) {
    ...
    if (this.cityCeil) {
      MAIN.interface.game.city.openMenu(MAIN.gameData.cities[this.type]);
    };
          ...
          thisBuilding.nearCeil.forEach((buildNearCeil, i) => {
            if (buildNearCeil == nearCeil || buildNearCeil === 'all') {
              if (building === 'road' || building === 'bridge') {
                ...
                if (MAIN.gameData.commonData.factoriesCount[building] > 0) {
                  buttons.push(building);
                };
              };
            };
