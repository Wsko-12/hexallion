...
function updateCreditHistory(){
  ...
  document.querySelector('#balanceMenu_balanceValue').innerHTML = MAIN.gameData.playerData.balance;
...
function calculateProductsWorth(){
  const player = MAIN.gameData.playerData;
  const cities = MAIN.gameData.cities;
  const prices = {};
...
function addBalanceMessage(message,amount){
  const statistics = MAIN.gameData.playerData.statistics;
  // thisStepProfit:0,
  // thisStepLose:0,
  //
  // lastStepProfit:0,
  ...
  if(MAIN.interface.lang.flag === 'ru'){
    if(message.startsWith('Tax payment')) {message = language.tax.ru + ' ' +MAIN.gameData.playerData.tax.procent + '%'};
    ...
    if(message.startsWith('Tax payment')) {message = message + ' ' + MAIN.gameData.playerData.tax.procent + '%'};
  }
...
function init(amount){
  ...
  const creditUser = MAIN.gameData.playerData.credit;
  ...
  const balanceHistorySection = `
    ...
    <div id="balanceMenu_Container">
      ...
      <div id="balanceMenu_balanceHistory" class="card">
        ...
        <div class="card-header">
            balance <span class="card-header-span"> | ${MAIN.gameData.playerData.login} </span>
        </div>

        <div id="balanceMenu_amount">
          $<span id="balanceMenu_balanceValue">${MAIN.gameData.playerData.balance}</span>
      ...
      if(balanceHistoryOpened){
        ...
        if(MAIN.tutorial.step === 'balance'){
          MAIN.tutorial.second({
            body:`
              Тут будет указана вся ваша информация по счету.
              </br>
              </br>
              В графе "продукция в обороте", показана сумма цен всех имеющихся у вас ресурсов (на складах, в грузовиках, в производстве)
              </br>
              </br>
              <span class="tutorialSection-important">Важно!</span>
              </br>
              Ваш баланс может уходить в минус, но тогда Вы не сможете ничего покупать или строить.
              Но как только Ваш баланс будет в минусе таком, как "продукция в обороте" — Вы проиграли.
            `,
            button:'Далее',
            fn(){
              if(balanceHistoryOpened){
...
function updatePayPerStep(){
  ...
  if(MAIN.gameData.playerData.credit.pays > 0 && MAIN.gameData.playerData.credit.deferment === 0){
    const credit = MAIN.gameData.playerData.credit;
    pay += (credit.amount / credit.allPays) + (credit.amount / credit.allPays) * (credit.procent / 100)
  };


  pay += MAIN.gameData.playerData.tax.value;

  div.innerHTML = pay;

  document.querySelector('#balanceHistory_tax').innerHTML = MAIN.gameData.playerData.tax.procent;
...
function change(newBalance){
  ...
  let startedBalance = MAIN.gameData.playerData.balance;
  MAIN.gameData.playerData.balance = newBalance;
  let curentAnimate = 0;
  const maxAnimate = 10;
